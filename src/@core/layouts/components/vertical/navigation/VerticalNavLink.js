...
import { extractSegment, handleURLQueries } from 'src/@core/layouts/utils'
...
const VerticalNavLink = ({
  ...
  const isNavLinkActive = () => {
    let route = `${activeAppRoute}${item.path}`
    if (router.pathname === route || handleURLQueries(router, item.path)) {
      return true
    } else {
      return false
    }
    ...
    <CanViewNavLink navLink={item}>
      ...
      >
        <MenuNavLink
          ...
          href={item.path === undefined ? '/' : `${activeAppRoute}${item.path}`}
