...
// ** Actions Imports
import { fetchUserPermissions } from 'src/store/apps/role&permissions'
...
const defaultColumns = [
  ...
  },
  {
    ...
    renderCell: ({ row }) => (
      <Typography sx={{ color: 'text.secondary' }}>{row.createdDate}</Typography>
    )
  }
]
...
const PermissionsTable = () => {
  ...
  const { permissionDetails } = crudProps && crudProps
  console.log('====rec result===', permissionDetails)
  ...
  const fetchUserPermissionTableData = () => {
    dispatch(
      fetchUserPermissions({
        limit: paginationModel.pageSize,
        page: paginationModel.page,
        token: auth.user.token,
        recordName: 'permissionDetails'
      })
    )
  }
  useEffect(() => fetchUserPermissionTableData(), [])
  return (
    <>
          ...
          <PageHeader
            ...
            subtitle={
              <Typography sx={{ color: 'text.secondary' }}>
                Each category (Basic, Professional, and Business) includes the four predefined roles
                shown below.
              </Typography>
            }
          />
